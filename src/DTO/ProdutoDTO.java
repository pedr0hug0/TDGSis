/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package DTO;

/**
 *
 * @author PeDr0_HuG0
 */
public class ProdutoDTO {
    
  private Integer id;
  private String codigo;
  private String descricao;
  private Integer qtd_cores;
  private Integer qtd_tamanho;
  private String cor;
  private Integer bordado;
  private Integer estampa;
  private Integer bolso;
  private String tipo_tamanho;
  private Integer frente;
  private Integer costas;
  private Integer manga;
  private Double rendimento;
  private String ref_alterar;
  private Integer especial;
  private String categoria;
  private String grade;

    public String getRef_alterar() {
        return ref_alterar;
    }

    public void setRef_alterar(String ref_alterar) {
        this.ref_alterar = ref_alterar;
    }

    /**
     * @return the codigo
     */
    public String getCodigo() {
        return codigo;
    }

    /**
     * @param codigo the codigo to set
     */
    public void setCodigo(String codigo) {
        this.codigo = codigo;
    }

    /**
     * @return the qtd_cores
     */
    public Integer getQtd_cores() {
        return qtd_cores;
    }

    /**
     * @param qtd_cores the qtd_cores to set
     */
    public void setQtd_cores(Integer qtd_cores) {
        this.qtd_cores = qtd_cores;
    }

    /**
     * @return the qtd_tamanho
     */
    public Integer getQtd_tamanho() {
        return qtd_tamanho;
    }

    /**
     * @param qtd_tamanho the qtd_tamanho to set
     */
    public void setQtd_tamanho(Integer qtd_tamanho) {
        this.qtd_tamanho = qtd_tamanho;
    }

    

    /**
     * @return the bordado
     */
    public Integer getBordado() {
        return bordado;
    }

    /**
     * @param bordado the bordado to set
     */
    public void setBordado(Integer bordado) {
        this.bordado = bordado;
    }

    /**
     * @return the estampa
     */
    public Integer getEstampa() {
        return estampa;
    }

    /**
     * @param estampa the estampa to set
     */
    public void setEstampa(Integer estampa) {
        this.estampa = estampa;
    }

    /**
     * @return the bolso
     */
    public Integer getBolso() {
        return bolso;
    }

    /**
     * @param bolso the bolso to set
     */
    public void setBolso(Integer bolso) {
        this.bolso = bolso;
    }

    /**
     * @return the descricao
     */
    public String getDescricao() {
        return descricao;
    }

    /**
     * @param descricao the descricao to set
     */
    public void setDescricao(String descricao) {
        this.descricao = descricao;
    }

    /**
     * @return the id
     */
    public Integer getId() {
        return id;
    }

    /**
     * @param id the id to set
     */
    public void setId(Integer id) {
        this.id = id;
    }

    /**
     * @return the tipo_tamanho
     */
    public String getTipo_tamanho() {
        return tipo_tamanho;
    }

    /**
     * @param tipo_tamanho the tipo_tamanho to set
     */
    public void setTipo_tamanho(String tipo_tamanho) {
        this.tipo_tamanho = tipo_tamanho;
    }

    /**
     * @return the frente
     */
    public Integer getFrente() {
        return frente;
    }

    /**
     * @param frente the frente to set
     */
    public void setFrente(Integer frente) {
        this.frente = frente;
    }

    /**
     * @return the costas
     */
    public Integer getCostas() {
        return costas;
    }

    /**
     * @param costas the costas to set
     */
    public void setCostas(Integer costas) {
        this.costas = costas;
    }

    /**
     * @return the manga
     */
    public Integer getManga() {
        return manga;
    }

    /**
     * @param manga the manga to set
     */
    public void setManga(Integer manga) {
        this.manga = manga;
    }

    /**
     * @return the rendimento
     */
    public Double getRendimento() {
        return rendimento;
    }

    /**
     * @param rendimento the rendimento to set
     */
    public void setRendimento(Double rendimento) {
        this.rendimento = rendimento;
    }

    /**
     * @return the especial
     */
    public Integer getEspecial() {
        return especial;
    }

    /**
     * @param especial the especial to set
     */
    public void setEspecial(Integer especial) {
        this.especial = especial;
    }

    /**
     * @return the cor
     */
    public String getCor() {
        return cor;
    }

    /**
     * @param cor the cor to set
     */
    public void setCor(String cor) {
        this.cor = cor;
    }

    /**
     * @return the categoria
     */
    public String getCategoria() {
        return categoria;
    }

    /**
     * @param categoria the categoria to set
     */
    public void setCategoria(String categoria) {
        this.categoria = categoria;
    }

    /**
     * @return the grade
     */
    public String getGrade() {
        return grade;
    }

    /**
     * @param grade the grade to set
     */
    public void setGrade(String grade) {
        this.grade = grade;
    }

    /**
     * @return the id
     */
    

}
